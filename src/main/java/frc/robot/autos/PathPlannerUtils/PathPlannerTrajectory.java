...
public class PathPlannerTrajectory extends Trajectory {
    ...
    private static List<State> generatePath(ArrayList<Waypoint> pathPoints, double maxVel, double maxAccel,
            boolean reversed) {
    ...
    private static void calculateVelocity(List<PathPlannerState> states, ArrayList<Waypoint> pathPoints,
            double maxAccel) {
    ...
    private static ArrayList<PathPlannerState> joinSplines(ArrayList<Waypoint> pathPoints, double maxVel, double step) {
        ...
        for (int i = 0; i < numSplines; i++) {
            ...
            for (double t = 0; t <= endStep; t += step) {
                Translation2d p = GeometryUtil.cubicLerp(startPoint.anchorPoint, startPoint.nextControl,
                ...
                if (i > 0 || t > 0) {
                    ...
                    double heading = Math.toDegrees(Math.atan2(s1.poseMeters.getY() - s2.poseMeters.getY(),
    ...
    public static class PathPlannerState extends State {
        ...
        private PathPlannerState interpolate(PathPlannerState endVal, double t) {
            ...
            lerpedState.accelerationMetersPerSecondSq = GeometryUtil.doubleLerp(accelerationMetersPerSecondSq,
                    endVal.accelerationMetersPerSecondSq, t);
            Translation2d newTrans = GeometryUtil.translationLerp(poseMeters.getTranslation(),
                    endVal.poseMeters.getTranslation(), t);
            Rotation2d newHeading = GeometryUtil.rotationLerp(poseMeters.getRotation(), endVal.poseMeters.getRotation(),
                    t);
            ...
            lerpedState.angularAcceleration = GeometryUtil.rotationLerp(angularAcceleration, endVal.angularAcceleration,
                    t);
            ...
            lerpedState.curvatureRadPerMeter = GeometryUtil.doubleLerp(curvatureRadPerMeter,
                    endVal.curvatureRadPerMeter, t);
